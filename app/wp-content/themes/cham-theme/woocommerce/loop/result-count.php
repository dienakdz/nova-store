...
<p class="woocommerce-result-count" role="status" aria-relevant="all" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'data-is-sorted-by="true"'; ?>>
	<?php
	if ( 1 === intval( $total ) ) {
		esc_html_e( 'Hiển thị 1 sản phẩm', 'cham-theme' );
		...
		printf(
			...
			esc_html__( 'Hiển thị tất cả %1$d sản phẩm', 'cham-theme' ) . $orderedby_placeholder,
			esc_html( $total ),
			esc_html( $orderedby )
		);
	} else {
		...
		printf(
			...
			esc_html__( 'Hiển thị %1$d-%2$d trong %3$d sản phẩm', 'cham-theme' ) . $orderedby_placeholder,
			esc_html( $first ),
			esc_html( $last ),
			esc_html( $total ),
			esc_html( $orderedby )
