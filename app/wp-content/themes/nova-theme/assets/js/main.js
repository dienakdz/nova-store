(function () {
	...
	function buildVariationSwatches() {
		document.querySelectorAll('form.variations_form table.variations select').forEach(function (select) {
			if (select.dataset.novaSwatches === 'ready') {
				return;
			}

			var wrapper = document.createElement('div');
			wrapper.className = 'nova-variation-swatches';
			...
			Array.prototype.slice.call(select.options).forEach(function (option) {
				...
				button.type = 'button';
				button.className = 'nova-variation-chip';
			...
			select.dataset.novaSwatches = 'ready';

			select.addEventListener('change', function () {
				wrapper.querySelectorAll('.nova-variation-chip').forEach(function (button) {
	...
	function syncSingleProductPrice() {
		document.querySelectorAll('form.variations_form').forEach(function (form) {
			if (form.dataset.novaPriceSync === 'ready') {
				return;
			}

			var price = document.querySelector('.nova-single-product-price');
			...
			form.dataset.novaPriceSync = 'ready';
	...
	function initBannerSliders() {
		document.querySelectorAll('[data-banner-slider]').forEach(function (slider) {
			if (slider.dataset.novaSlider === 'ready') {
				return;
			}
			...
			slider.dataset.novaSlider = 'ready';
			showSlide(0);
			start();
		});
	}
